...
$('#goalForm').onsubmit=()=>{const target=$('#goalType').value==='monthly'?'#monthlyGoal':'#weeklyGoal';$(target).value=$('#goalText').value.trim();saveGoals()};
...
function renderCalendar(){
  ...
  for(let i=0;i<cellCount;i++){
    ...
    matches.slice(0,calendarMode==='week'?8:3).forEach(e=>{const b=document.createElement('button');b.className=`event-chip ${e.type==='task'?'task-chip':''} ${e.completed?'completed':''}`;const color=calendarColors[e.category]||'#557bea';b.style.borderColor=color;b.style.background=hexAlpha(color,'18');b.style.color=color;const range=e.time+(e.endTime?'–'+e.endTime:'');const prefix=e.type==='task'?(e.completed?'☑ ':`☐ <time>${formatRemaining(e)}</time>`):(!e.allDay&&e.time?`<time>${range}</time>`:e.allDay?'<time>終日</time>':'');b.innerHTML=`${prefix}${esc(e.title)}`;b.onclick=x=>{x.stopPropagation();openEvent(e)};cell.appendChild(b)});
    if(matches.length>3)cell.insertAdjacentHTML('beforeend',`<div class="more-events">他 ${matches.length-3} 件</div>`);
...
$('#newEventButton').onclick=()=>openEvent();$('#newNoteButton').onclick=()=>openNote();
...
$$('.nav-item').forEach(b=>b.onclick=()=>{$$('.nav-item').forEach(x=>x.classList.remove('active'));b.classList.add('active');$$('.view').forEach(v=>v.classList.remove('active-view'));$(`#${b.dataset.view}View`).classList.add('active-view')});
...
function renderSidebarTasks(){const tasks=events.filter(e=>e.type==='task'&&!e.completed).sort((a,b)=>taskDeadline(a)-taskDeadline(b));$('#openTaskCount').textContent=`${tasks.length}件`;$('#sidebarTaskList').innerHTML=tasks.slice(0,5).map(t=>{const overdue=taskDeadline(t)<Date.now(),color=calendarColors[t.category]||'#557bea';return `<button class="sidebar-task ${overdue?'overdue':''}" data-sidebar-task="${t.id}" style="--task-color:${color};--task-bg:${hexAlpha(color,'14')}"><span><i style="background:${color}"></i>${esc(t.title)}</span><small>${formatRemaining(t)}</small></button>`}).join('')||'<div class="sidebar-task-empty">未完了タスクはありません</div>';$$('[data-sidebar-task]').forEach(b=>b.onclick=()=>openEvent(events.find(e=>e.id===b.dataset.sidebarTask)))}
